...
The server `erdblick` object uses the same keys as `config.json`: `styles`, `mapPresetsEnabled`, `mapPresets`, `extensionModules`, `surveys`, `backgroundLayers`, `defaultBackgroundLayerId`, `coordinates-enabled`, `coordinates-legal-terms`, and optional `state`. Empty arrays, empty objects, and empty strings are normally treated as absent. A present server `mapPresets` list replaces the bundled list, and `mapPresets: []` explicitly clears it.
...
- `config/config.json` lists built-in style bundles and optional extension modules. Common keys:
  ...
  - `mapPresetsEnabled`: optional Boolean map-preset feature switch. It defaults to `true`; explicit `false` disables selection and editing while keeping the **Map Presets** tab visible with: “Map presets are currently disabled. Modify the configuration to enable them.”
...
The effective `mapPresets` list is configuration-owned and is not AppState or browser storage. With the default `mapPresetsEnabled: true`, key omission means an enabled empty catalog. A present server list replaces the static list, while server omission inherits static configuration. Explicit `mapPresetsEnabled: false` disables runtime presets and editing but leaves the management tab visible with an explanatory message. Valid survivors from malformed server input remain usable when the feature is enabled, but editing is disabled until the YAML is repaired.

Native MapViewer deployments may advertise a narrow, revision-guarded map-preset write capability when `mapPresetsEnabled` is true, `allow-post-config` is enabled, and the effective catalog is valid. The list does not need to exist initially: the first Add creates `erdblick.mapPresets` in the durable YAML. Add, availability toggles, and raw Apply replace only that complete YAML list using `If-Match`; conflicts refetch the authoritative catalog. Static-only deployments and Python-launched Docker sessions are read-only because the latter mounts a disposable transformed YAML copy. Per-view selected map/layer presets remain ordinary browser state.
