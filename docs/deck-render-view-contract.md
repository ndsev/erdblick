...
- `app/mapview/deck/navigation/web-mercator-feature-navigation.ts` owns Erdblick's scalar maximal-safe close-feature zoom policy and settled ground-centre conversion. Active camera reconstruction is delegated to deck.gl's public `WebMercatorViewport` planar-pan and orbit/zoom operations.
