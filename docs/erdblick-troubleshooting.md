...
- Reduce the number of active style sheets or expensive debug overlays.
...
- Use the style dialog’s **Reset**, compare, and import/export actions, or the “Clear” buttons for imported styles and modified built‑in styles in Preferences, to get back to a known state.
- Use the style editor to catch YAML syntax errors before saving.
- If the problem affects only one view, check the per-layer style options in **Maps & Layers** before editing the YAML.
...
3. If that is not enough, clear the full site data via your browser's developer tools.
4. Reload the page. Step 2 clears viewer/search state only; clearing full site data also removes imported styles and modified built-in styles.
...
- The exported log/diagnostics data of your session (see [Diagnostics Docs](erdblick-diagnostics.md))
- Screenshot of **Help -> About**, including the version table if available.
