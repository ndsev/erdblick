...
Use **Save** beside the high-fidelity controls to create a canonical stylesheet. The small save dialog accepts the exact stylesheet name, an **Enabled by default** choice, and optional layer affinity. Slashes in the name create the same groups used by the normal Styles tree. Empty affinity means any feature layer; selected layer IDs are stored as an escaped exact-match `layer` expression and apply to every map that uses that ID. Saving is create-only: an empty rule list or a name/URL collision is rejected without overwriting anything. The generated source has `category: search`, `version: 2`, the chosen Boolean `default`, and one flat top-level YAML rule per GUI rule.
...
The adjacent **Saved styles** menu lists loaded stylesheets whose category is `search`. Selecting one projects every compatible rule into the current GUI as a detached copy. Root `default` and `layer` metadata govern ordinary stylesheet rendering; they do not alter the active search scope when rules are copied here. Editing that copy does not change the stylesheet, and later stylesheet edits do not change the search. A partially compatible stylesheet remains selectable and reports every rule omitted from the copy. A stylesheet with no compatible rules is shown in red and disabled.

Edit a saved stylesheet from the ordinary Styles tree. The Style Editor opens on **Quick**, which reuses the rule controls, and also provides **Advanced** for authoritative YAML editing. Quick changes update Advanced immediately. Rules or properties outside the current controls are preserved; warnings below Quick identify preserved or read-only content. Apply the editor changes to update the imported stylesheet, or use the ordinary **Export** action to download it.
