...
function overlay(id: string, featureIds: readonly string[]): TileSubsetInteractionOverlay {
    return {
        id,
        effect: EFFECT,
        order: 10,
...
describe("GpuSceneMaskController", () => {
    ...
    it("coalesces owners into one sparse target group and geometry-faithful pass", () => {
        ...
        controller.setOverlays("owner-a", new Set(["a"]), [11, 48, 0], [
        ...
        expect([...outline.masks.values()].map(mask => mask.sourceId))
            .toEqual(["scene-vector"]);
        for (const mask of outline.masks.values()) {
            expect(mask.layer).toBeInstanceOf(ErdblickVectorMaskLayer);
            expect(mask.layer.id).toBe(
                `${mask.groupId}/${mask.sourceId}`
            );
            expect((mask.layer.props as {drillPickEligible?: boolean})
                .drillPickEligible).toBe(false);
        }
        const sparseWrites = device.textures[0].writes.filter(write =>
    ...
    it("groups equal style glows and filters their union by material key", () => {
        ...
        const configuration = (vector as unknown as {
            configuration: {
