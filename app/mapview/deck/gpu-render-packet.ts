...
export enum GpuMaterialFlag {
  ...
  SimplePath = 1 << 3,
  DualStrokePath = 1 << 4,
  ScreenLengthStartAnchor = 1 << 5,
  ScreenLengthEndAnchor = 1 << 6,
  ScreenLengthCenterAnchor = 1 << 7,
  SemanticSupport = 1 << 8,
  SemanticOverlay = 1 << 9,
  PointRing = 1 << 10,
  SurfaceShading = 1 << 11,
...
export class GpuRetainedPickTable {
  ...
  constructor(picks: Uint8Array) {
    ...
    this.storage = {
      picks: new DataView(picks.buffer, picks.byteOffset, picks.byteLength),
...
export class GpuPickTableView {
  ...
  contributionIndex(index: number): number {
    return this.storage.picks.getUint32(pickOffset(this.storage, index), true);
  }
...
export class GpuRenderPacketView {
  ...
  constructor(packet: Uint8Array) {
    if (
      packet.byteLength < GPU_RENDER_PACKET_HEADER_BYTES ||
    ...
    if (
      this.u16(4) !== GPU_RENDER_PACKET_ABI_VERSION ||
  ...
  private readStreams(table: PacketTable): GpuRecordStreamView[] {
    const expectedStrides: Record<number, number> = {
      ...
      [GpuPrimitiveKind.PathSegment]: 148,
    ...
    return Array.from({ length: table.count }, (_, index) => {
      ...
      const compactPath = (flags & GpuMaterialFlag.CompactPath) !== 0;
      ...
      const screenLengthAnchor =
        ...
        (GpuMaterialFlag.ScreenLengthStartAnchor |
          GpuMaterialFlag.ScreenLengthEndAnchor |
          GpuMaterialFlag.ScreenLengthCenterAnchor);
      const validScreenLengthAnchor =
        screenLengthAnchor === 0 ||
        screenLengthAnchor === GpuMaterialFlag.ScreenLengthStartAnchor ||
        screenLengthAnchor === GpuMaterialFlag.ScreenLengthEndAnchor ||
        screenLengthAnchor === GpuMaterialFlag.ScreenLengthCenterAnchor;
      ...
      if (
        ...
        (flags &
          ~(
            GpuMaterialFlag.Billboard |
            GpuMaterialFlag.DepthTest |
            GpuMaterialFlag.CompactPath |
            GpuMaterialFlag.SimplePath |
        ...
        ((flags & GpuMaterialFlag.SemanticSupport) !== 0 &&
          (flags & GpuMaterialFlag.SemanticOverlay) !== 0)
      ) {
  ...
  private readSpans(table: PacketTable): GpuContributionSpanView[] {
    return Array.from({ length: table.count }, (_, index) => {
      ...
      if (
        !stream ||
        result.firstRecord > stream.recordCount ||
  ...
  private readContributions(
    ...
    for (let index = 0; index < table.count; ++index) {
      ...
      if (
        firstSpan !== expectedSpan ||
        firstSpan > spans.length ||
        spanCount > spans.length - firstSpan ||
        firstPick !== expectedPick ||
        firstPick > pickCount ||
        ownedPickCount > pickCount - firstPick ||
        firstLabel !== expectedLabel ||
        firstLabel > labelCount ||
        ownedLabelCount > labelCount - firstLabel ||
        firstZIndex !== expectedZIndex ||
        firstZIndex > zIndexTable.count ||
        zIndexCount > zIndexTable.count - firstZIndex
      ) {
        throw new Error(
          `GPU contribution ${index} has an invalid owned range.`,
        );
      }
      const ownedSpans = spans.slice(firstSpan, firstSpan + spanCount);
      if (
        ownedSpans.some((span) => span.recordCount === 0) ||
        new Set(ownedSpans.map((span) => span.streamIndex)).size !==
          ownedSpans.length
      ) {
        throw new Error(
          `GPU contribution ${index} has an invalid stream span.`,
        );
      }
      result.push({
        key,
        ...
        zIndices: Array.from({ length: zIndexCount }, (_, ordinal) => {
          const offset = zIndexTable.offset + (firstZIndex + ordinal) * 16;
          const value = this.f64(offset);
          ...
          return {
            ...
            semanticGroup: this.u32(offset + 12),
          };
        }),
      });
    ...
    if (
      expectedSpan !== spans.length ||
      expectedPick !== pickCount ||
      expectedLabel !== labelCount ||
      expectedZIndex !== zIndexTable.count
    ) {
  ...
  private validateContributionOwnership(): void {
    ...
    for (let index = 0; index < this.contributions.length; ++index) {
      ...
      for (
        let pick = owner.firstPick;
        pick < owner.firstPick + owner.pickCount;
        ++pick
      ) {
        ...
        if (
          ...
          throw new Error(
            `GPU pick ${pick} disagrees with its contribution range.`,
          );
        }
      }
      if (
        ...
        (owner.pickCount !== owner.totalPickCount ||
          (owner.pickCount > 0 &&
            this.picks.localPickIndex(owner.firstPick + owner.pickCount - 1) +
              1 !==
              owner.totalPickCount))
      ) {
        throw new Error(
      ...
      for (
        let label = owner.firstLabel;
        label < owner.firstLabel + owner.labelCount;
        ++label
      ) {
        const record = this.labels[label];
        if (
          record?.contributionIndex !== index ||
          (record.localPickIndex !== 0xffffffff &&
            record.localPickIndex >= owner.totalPickCount)
        ) {
          throw new Error(
            `GPU label ${label} disagrees with its contribution range.`,
          );
        }
      }
    }
    for (
      ...
      const ranges = rangesByStream[streamIndex].sort(
        (left, right) => left.firstRecord - right.firstRecord,
      );
      let cursor = 0;
  ...
  private f32(offset: number): number {
    return this.data.getFloat32(offset, true);
  }
  private f64(offset: number): number {
