...
function pointPacket(
  ...
  view.setUint32(contributionOffset + 16, contribution.activationToken, true);
...
describe("GpuScene contribution lifecycle", () => {
  ...
  it("releases a new contribution when reservation allocation fails", () => {
    const { scene } = createScene();
    (scene as unknown as { nextActivationToken: number }).nextActivationToken =
      0x00ff_ffff;
    ...
    expect(
      ...
      ).contributionSlots.highWaterRecord,
    ).toBe(0);
  });
  ...
  it("separates distinct semantic ties at the same authored depth", () => {
    const { device, scene } = createScene();
    for (const [tileId, depthTieKey] of [
      [1, 17],
      [2, 18],
    ]) {
      const reservation = scene.prepareRender(
  ...
  it("does not invent tie depth for un-authored geometry", () => {
    const { device, scene } = createScene();
    const install = (
      identity: string,
      tileId: number,
      ...
      scene.applyPacket(
        pointPacket(reservation, { zIndex, depthTieKey }),
        reservation,
      );
      scene.finishRender(reservation);
    };
    install("un-authored-a", 1, 17);
    install("un-authored-b", 2, 18);
    install("authored", 3, 19, 10);
  ...
  it("rejects a superseded packet without replacing the visible revision", () => {
    ...
    expect(
      scene.applyPacket(pointPacket(stale, { pointByte: 2 }), stale),
    ).toMatchObject({ appliedContributions: 0 });
    expect(scene.snapshot().activeContributionCount).toBe(1);
    expect(
      scene.applyPacket(pointPacket(current, { pointByte: 3 }), current),
  ...
  it("keeps the active revision isolated from a failed replacement upload", () => {
    ...
    const replacement = scene.prepareRender("origin", [11, 48, 0], input);
    expect(replacement.contributions[0].slot).not.toBe(
      initial.contributions[0].slot,
    );
  ...
  it("publishes a storage generation when inactive staging grows a shared buffer", () => {
    ...
    const staging = scene.prepareRender(
      "origin",
      [11, 48, 0],
      [
        {
  ...
  it("recycles contribution and origin slots only after all owners release", () => {
    const { scene } = createScene();
    const first = scene.prepareRender(
      "first-origin",
      [11, 48, 0],
      [
        {
    ...
    const second = scene.prepareRender(
      "second-origin",
      [12, 49, 0],
      [
        {
  ...
  it("keeps stale holes inert when their contribution slot is recycled", () => {
    ...
    expect(replacement.contributions[0].activationToken).not.toBe(
