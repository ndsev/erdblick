...
const GEOMETRY_ORDER: FeatureSearchGeometryKind[] = ["any", "point", "line", "surface", "polygon", "mesh", "label"];
const VALID_GEOMETRIES = new Set<FeatureSearchGeometryKind>(GEOMETRY_ORDER);
...
function normalizeGeometryKind(value: unknown): FeatureSearchGeometryKind | undefined {
    if (typeof value !== "string") {
        return undefined;
    }
...
/**
 * Normalizes legacy scalar and current multi-geometry rule values. `Any` and
 * `Label` stay exclusive because labels use a different native presentation.
 */
export function normalizeFeatureSearchGeometry(value: unknown): FeatureSearchGeometryKind[] {
    const values = Array.isArray(value) ? value : [value];
    const selected = new Set(values
        .map(normalizeGeometryKind)
        .filter((geometry): geometry is FeatureSearchGeometryKind => geometry !== undefined));
    if (selected.has("any")) {
        return ["any"];
    }
    ...
    if (selected.has("surface")) {
        selected.delete("polygon");
        selected.delete("mesh");
    }
    const normalized = GEOMETRY_ORDER.filter(geometry => selected.has(geometry));
    return normalized.length ? normalized : ["any"];
}
...
export function normalizeFeatureSearchStyleRule(value: unknown): FeatureSearchStyleRule | null {
    ...
    return {
        ...
        geometry: normalizeFeatureSearchGeometry(raw["geometry"] ?? raw["type"]),
