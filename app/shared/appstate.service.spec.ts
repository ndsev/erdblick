...
describe('AppStateService', () => {
    ...
    it('falls back to v1 URL hydration for invalid v2 flag values', async () => {
        const routerStub = createRouterStub({ v2: 'wat', m: '1' });
        const infoServiceStub = {
            showError: vi.fn(),
            showSuccess: vi.fn(),
            showWarning: vi.fn(),
        ...
        expect(service.markerState.getValue()).toBe(true);
        expect(infoServiceStub.showAlertDialogDefault).not.toHaveBeenCalled();

        service.ngOnDestroy();
        routerStub.events.complete();
    });

    it('cancels pending URL sync before popstate hydration to avoid schema races', async () => {
        vi.useFakeTimers();
        ...
        routerStub.routerState.snapshot.root!.queryParams = { v2: '1', m: '0' };
        // @ts-expect-error this is a call to mock router
        routerStub.getCurrentNavigation.mockReturnValue({ trigger: 'popstate' });
        routerStub.events.next(new NavigationStart(2, '/?v2=1&m=0'));
        routerStub.events.next(new NavigationEnd(2, '/?v2=1&m=0', '/?v2=1&m=0'));
        await flushMicrotasks();
    ...
    it('switches URL writer mode when navigating from v2 to v1 via popstate', async () => {
        const routerStub = createRouterStub({ v2: '1', m: '1' });
        const infoServiceStub = {
            showError: vi.fn(),
            showSuccess: vi.fn(),
            showInfo: vi.fn(),
        ...
        const service = new AppStateService(routerStub as unknown as Router, infoServiceStub);
        routerStub.events.next(new NavigationEnd(1, '/?v2=1&m=1', '/?v2=1&m=1'));
        await flushMicrotasks();
        ...
        expect(service.markerState.getValue()).toBe(false);
        expect((service as any).currentUrlVersion).toBe(1);

        service.markerState.next(true);
        await flushMicrotasks();

        // @ts-expect-error this is a call to mock router
        const lastCall = routerStub.navigate.mock.calls.at(-1)?.[1];
        const queryParams = lastCall?.queryParams ?? {};
        expect(lastCall?.queryParamsHandling).toBe('merge');
        expect(queryParams.m).toBe('1');
        expect(queryParams.v2).toBeUndefined();
        expect(infoServiceStub.showInfo).toHaveBeenNthCalledWith(
            1,
            'URL schema version 2 is currently in use. You can switch the URL schema in the settings.'
        );
        expect(infoServiceStub.showInfo).toHaveBeenNthCalledWith(
            2,
            'URL schema version 1 is currently in use. You can switch the URL schema in the settings.'
        );
    ...
    it('does not show schema toast when switching URL schema in settings', async () => {
        const routerStub = createRouterStub({ m: '1' });
        const infoServiceStub = {
            showError: vi.fn(),
            showSuccess: vi.fn(),
        ...
        const service = new AppStateService(routerStub as unknown as Router, infoServiceStub);
        routerStub.events.next(new NavigationEnd(1, '/?m=1', '/?m=1'));
        await flushMicrotasks();

        expect(infoServiceStub.showInfo).not.toHaveBeenCalled();

        service.useUrlV2 = true;
        await flushMicrotasks();
        service.useUrlV2 = false;
        await flushMicrotasks();
    ...
    it('does not show schema toast on reload when persisted schema matches URL schema', async () => {
    ...
    it('shows schema toast when URL auto-switches from persisted v2 to v1', async () => {
        ...
        expect((service as any).currentUrlVersion).toBe(1);
        expect(infoServiceStub.showInfo).toHaveBeenCalledWith(
            'URL schema version 1 is currently in use. You can switch the URL schema in the settings.'
        );
    ...
    it('allows manually switching URL writer mode between v1 and v2', async () => {
        const routerStub = createRouterStub({ m: '1' });
        const infoServiceStub = {
            showError: vi.fn(),
            showSuccess: vi.fn(),
        ...
        // @ts-expect-error this is a call to mock router
        const v2Call = routerStub.navigate.mock.calls.at(-1)?.[1];
        expect((service as any).currentUrlVersion).toBe(2);
        expect(v2Call?.queryParamsHandling).toBe('replace');
        expect(v2Call?.queryParams?.v2).toBe('1');

        service.useUrlV2 = false;
        await flushMicrotasks();

        // @ts-expect-error this is a call to mock router
        const v1Call = routerStub.navigate.mock.calls.at(-1)?.[1];
        expect((service as any).currentUrlVersion).toBe(1);
        expect(v1Call?.queryParamsHandling).toBe('replace');
        expect(v1Call?.queryParams?.v2).toBeUndefined();
        expect(v1Call?.queryParams?.m).toBe('1');

        service.ngOnDestroy();
        routerStub.events.complete();
    });

    it('hydrates compact v2 search, selection, and singleton camera values', async () => {
        const routerStub = createRouterStub({
            ...
            p: '-1.57',
            r: '0',
            tk: 'a:F:map:layer:tile;b:SD:m1:SD-LAYER:tile',
            sel: '1~d~@a~feat-1~n0,2~u~@b~~@a~feat-2~30:40~n1',
        });
        const infoServiceStub = {
            showError: vi.fn(),
            showSuccess: vi.fn(),
        ...
        expect(service.searchState.getValue()).toEqual([1, 'speed']);
        expect(service.cameraViewDataState.getValue(0).destination.lon).toBe(11.1);
        expect(service.cameraViewDataState.getValue(1).destination.lon).toBe(11.1);

        expect(service.selection).toHaveLength(2);
        expect(service.selection[0].locked).toBe(true);
        expect(service.selection[0].undocked).toBe(false);
        expect(service.selection[0].color).toBe('#fff314');
        expect(service.selection[0].features[0]).toEqual({
            mapTileKey: 'Features:map:layer:tile',
            featureId: 'feat-1',
        });

        expect(service.selection[1].sourceData?.mapTileKey).toBe('SourceData:m1:SourceData-LAYER:tile');
        expect(service.selection[1].size).toEqual([30, 40]);
        expect(service.selection[1].color).toBe('#4ad6d6');

        service.ngOnDestroy();
        routerStub.events.complete();
    });

    it('hydrates compact v2 selection with layer-group indices', async () => {
        const routerStub = createRouterStub({
            v2: '1',
            l: 'Lane@m1,m2,m3',
            sel: '1~d~F:0:2:21fa0777000d:0~Lane.1~n0',
        });
        const infoServiceStub = {
            showError: vi.fn(),
            showSuccess: vi.fn(),
        ...
        expect(service.selection[0].features).toHaveLength(1);
        expect(service.selection[0].features[0]).toEqual({
            mapTileKey: 'Features:m3:Lane:21fa0777000d:0',
            featureId: 'Lane.1',
        });
    ...
    it('syncs v2 URLs as full replace snapshots when v2=1 is active', async () => {
        const routerStub = createRouterStub({ v2: '1' });
        const infoServiceStub = {
            showError: vi.fn(),
            showSuccess: vi.fn(),
            showWarning: vi.fn(),
        ...
        expect(routerStub.navigate).toHaveBeenCalledWith([], expect.objectContaining({
            queryParams: expect.objectContaining({ v2: '1', m: '1' }),
    ...
    it('hydrates grouped v2 layer names before style option decoding', async () => {
        const routerStub = createRouterStub({
            v2: '1',
            l: 'Lane@m1,m2,m3~POI@m3',
            'ABCD~1~.Lanes': '1',
        });
        const infoServiceStub = {
            showError: vi.fn(),
        ...
        const styleKey = service.stylesState.styleOptionKey('m2', 'Lane', 'ABCD', 'showLanes');
        expect(service.styles.get(styleKey)).toEqual(['1']);

        service.ngOnDestroy();
        routerStub.events.complete();
    });

    it('compacts v2 layer universe to referenced indices and remaps style keys', async () => {
        const routerStub = createRouterStub({ v2: '1' });
        const infoServiceStub = {
            showError: vi.fn(),
            showSuccess: vi.fn(),
        ...
        service.layerNamesState.next(['mA/layerA', 'mB/layerB', 'mC/layerC']);
        service.layerZoomLevelState.next(0, [13, 7, 13]);
        service.setStyleOptionValues('mC', 'layerC', 'ABCD', 'showLanes', [true]);
        await flushMicrotasks();

        // @ts-expect-error this is a call to mock router
        const lastCall = routerStub.navigate.mock.calls.at(-1)?.[1];
        const queryParams = lastCall?.queryParams ?? {};
        expect(queryParams.v2).toBe('1');
        expect(queryParams.l).toBe('layerB@mB~layerC@mC');
        expect(queryParams.z).toBe('7,13');
        expect(queryParams['ABCD~1~.Lanes']).toBe('1');
        expect(Object.keys(queryParams)).not.toContain('ABCD~2~showLanes');

        service.ngOnDestroy();
        routerStub.events.complete();
    });

    it('keeps layer-count semantics when compacting homogeneous v2 visibility vectors', async () => {
        const routerStub = createRouterStub({ v2: '1', n: '2' });
        const infoServiceStub = {
            showError: vi.fn(),
            showSuccess: vi.fn(),
            showWarning: vi.fn(),
        ...
        service.layerVisibilityState.next(0, [true, true, true, true, true]);
        service.layerVisibilityState.next(1, [true, true, true, true, true]);
        await flushMicrotasks();

        // @ts-expect-error this is a call to mock router
        const lastCall = routerStub.navigate.mock.calls.at(-1)?.[1];
        const queryParams = lastCall?.queryParams ?? {};
        expect(queryParams.v).toBe('1x5');

        service.ngOnDestroy();
        routerStub.events.complete();
    });

    it('encodes v2 feature selections with layer-group indices derived from l', async () => {
        const routerStub = createRouterStub({ v2: '1' });
        const infoServiceStub = {
            showError: vi.fn(),
            showSuccess: vi.fn(),
        ...
        service.layerNamesState.next(['m1/Lane', 'm2/Lane']);
        service.selectionState.next([{
            id: 0,
            features: [
                {mapTileKey: 'Features:m1:Lane:tileA:0', featureId: 'Lane.24'},
                {mapTileKey: 'Features:m2:Lane:tileB:0', featureId: 'Boundary.138.0'},
            ],
            locked: true,
            size: [30, 20],
            color: '#fff314',
        ...
        const lastCall = routerStub.navigate.mock.calls.at(-1)?.[1];
        const queryParams = lastCall?.queryParams ?? {};
        expect(queryParams.l).toBe('Lane@m1,m2');
        expect(queryParams.sel).toContain('F:0:0:tileA:0');
        expect(queryParams.sel).toContain('F:0:1:tileB:0');

        service.ngOnDestroy();
        routerStub.events.complete();
    });

    it('hydrates run-length visibility vectors without dropping layer activations', async () => {
        const routerStub = createRouterStub({
            v2: '1',
            n: '2',
            l: 'Lane@m1,m2,m3,m4,m5',
            v: '1x5',
        });
        const infoServiceStub = {
            showError: vi.fn(),
        ...
        expect(service.layerVisibilityState.getValue(0)).toEqual([true, true, true, true, true]);
        expect(service.layerVisibilityState.getValue(1)).toEqual([true, true, true, true, true]);

        service.ngOnDestroy();
        routerStub.events.complete();
    });

    it('keeps v2 layer names when no explicit layer references are present', async () => {
        const routerStub = createRouterStub({ v2: '1' });
        const infoServiceStub = {
            showError: vi.fn(),
            showSuccess: vi.fn(),
        ...
        service.layerNamesState.next(['m1/layerA', 'm2/layerB']);
        await flushMicrotasks();
        ...
        const queryParams = lastCall?.queryParams ?? {};
        expect(queryParams.v2).toBe('1');
        expect(queryParams.l).toBe('layerA@m1~layerB@m2');

        service.ngOnDestroy();
        routerStub.events.complete();
    });

    it('uses v2 hidden fallback only for layers omitted from URL layer names', async () => {
        const routerStub = createRouterStub({ v2: '1', l: 'm1/layerA' });
        const infoServiceStub = {
            showError: vi.fn(),
            showSuccess: vi.fn(),
            showWarning: vi.fn(),
        ...
        const existing = service.mapLayerConfig('m1', 'layerA', true, 9);
        const omitted = service.mapLayerConfig('m2', 'layerB', true, 9);

        expect(existing[0].visible).toBe(true);
        expect(omitted[0].visible).toBe(false);
